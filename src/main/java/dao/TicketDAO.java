...
public class TicketDAO {
    ...
    public void modificarEstatus() throws ClassNotFoundException {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(null, "Se ha reiniciado la numeración del ticket");
            numero = 1;
            insertarTicket();
    ...
    public ResultSet getTickets(Date fecha) {
        ...
        try {
            Connection conn = SQL.conectarMySQL();
            Statement s = conn.createStatement();
            datos = s.executeQuery("SELECT idTicket,fecha,hora FROM `Ventas` where fecha = '" + formatoFecha.format(fecha) + "' and devolucion = false  and devocompleta=false GROUP by hora");
    ...
    public void eliminarVentasDeTicket(String idTicket) {
        try {
            Connection conn = SQL.conectarMySQL();
            Statement s = conn.createStatement();
            Statement s2 = conn.createStatement();
            String sqlPorTicket = "Select codigo, cantidad from ventas where idTicket = " + idTicket;
    ...
    public void eliminarVentasDeTicketPorProducto(String idTicket, String codigo, String monto, double cantidad, int opcion, String canAgregar) {
        ...
        try {
            ...
            if (opcion == 1) {
               ...
               String sql = "UPDATE `Ventas` SET devolucion = true, devocompleta = true where idTicket = " + idTicket + " and codigo = '" + codigo + "'";
                s.executeUpdate(sql);
              ...
              cantidadTotal.next();
                ...
                String sql = "UPDATE `Ventas` SET devolucion = true, cantidad = " + cantidad + " where idTicket = " + idTicket + " and codigo = '" + codigo + "';";
            ...
            conn.commit();
            JOptionPane.showMessageDialog(null, "Se ha realizado la devolción");
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(TicketDAO.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SQLException ex) {
            Logger.getLogger(TicketDAO.class.getName()).log(Level.SEVERE, null, ex);
