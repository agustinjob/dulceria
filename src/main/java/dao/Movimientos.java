...
public class Movimientos {
      ...
      public ResultSet obteneEfectivoInicial() throws ClassNotFoundException, SQLException {
        Conexion SQL = new Conexion();
        Connection conn = SQL.conectarMySQL();
        Statement s = conn.createStatement();
        ResultSet rs = s.executeQuery("SELECT * FROM `Movimientos_extras`  where tipo= 'efectivo_inicial' and fecha= '" + formatoFecha.format(d) + "'");
      ...
      public ResultSet getEntradasDelDia(String tipo) throws ClassNotFoundException, SQLException{
        ...
        Connection conn = SQL.conectarMySQL();
        Statement s = conn.createStatement();
        ResultSet rs = s.executeQuery("SELECT * FROM `Movimientos_extras` where tipo= '"+ tipo +"' and fecha= '" + formatoFecha.format(d)+"'");
        rs.last();
        if (rs.getRow() == 0) {
            rs=null;
        } else {
...
" AND YEAR(fecha) = "+year +" group by fecha");
       ...
       public ResultSet getDevoluciones() throws ClassNotFoundException, SQLException{      
        ...
        Connection conn = SQL.conectarMySQL();
        Statement s = conn.createStatement();
        ResultSet rs = s.executeQuery("SELECT sum(monto) as devoluciones FROM `Movimientos_extras` where tipo= 'devolución' and fecha= '" + formatoFecha.format(d)+"'");
        rs.last();
        if (rs.getRow() == 0) {
            rs=null;
        } else {
