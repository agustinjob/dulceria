...
public class UsuarioDAO {
    ...
    public void almacena( String [] a) throws ClassNotFoundException, SQLException {
        ...
        int rs = s.executeUpdate("INSERT INTO `Usuario`(`nombre`, `direccion`, `username`, `password`, `tipoUsuario`, `telefono`) "
   ...
   String mensajeBita ="Registro al usuario " + a[0];
    ...
    public void modificarDatosUsuario(Usuario u){
     Connection conn;
        ...
        System.out.println("UPDATE `Usuario` SET `nombre`='"+u.getNombre()+"',`direccion`='"+u.getDireccion()+"',`username`='"+u.getUsername()+"',`password`='"+u.getPassword()+"',"
                + "`tipoUsuario`= '"+u.getTipo()+"' ,`telefono`='"+u.getTelefono()+"' WHERE idUsuario = "+ u.getIdUsuario());
    ...
    public void modificarFechaYhoraSesionUsuario(int idUsuario){
    Connection conn;
        try {
            conn = SQL.conectarMySQL();
            Statement s = conn.createStatement();
     ...
     String mensajeBita ="Elimino los datos del usuario " + u.getNombre();
            bita.registrarBitacora(mensajeBita);
            JOptionPane.showMessageDialog(null,"Datos del usuario eliminados");
        } catch (ClassNotFoundException ex) {
            Logger.getLogger(UsuarioDAO.class.getName()).log(Level.SEVERE, null, ex);
        } catch (SQLException ex) {
            Logger.getLogger(UsuarioDAO.class.getName()).log(Level.SEVERE, null, ex);
